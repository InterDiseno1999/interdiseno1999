...
<div class="max-w-5xl mx-auto px-2 sm:px-4">
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="productForm">
        ...
        <div class="bg-[#C0B7B1]/40 rounded-3xl md:rounded-[2.5rem] p-6 md:p-12 border border-gray-100 shadow-sm space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10">
                <div class="space-y-3">
                    ...
                    <textarea name="description" required rows="8" 
                        class="w-full p-4 rounded-3xl border-none shadow-sm focus:ring-2 focus:ring-black outline-none text-base resize-none bg-white/80 transition-all">{{ old('description', $product->description) }}</textarea>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center gap-4 bg-white/90 p-4 rounded-2xl shadow-sm border border-gray-50">
                            <button type="button" id="stockToggleButton"
                                class="relative inline-flex h-7 w-12 items-center rounded-full transition-colors duration-300 focus:outline-none shadow-inner {{ $product->stock ? 'bg-green-500' : 'bg-gray-300' }}">
                                <span id="stockToggleCircle" class="{{ $product->stock ? 'translate-x-6' : 'translate-x-1' }} inline-block h-5 w-5 transform rounded-full bg-white transition-transform duration-300 shadow-md"></span>
                            </button>
                            <span id="stockStatusText" class="font-bold text-xs md:text-sm uppercase tracking-tight {{ $product->stock ? 'text-green-600' : 'text-gray-400' }}">
                                {{ $product->stock ? 'Disponible' : 'Sin Stock' }}
                            </span>
                            <input type="hidden" name="stock" id="stockHiddenInput" value="{{ $product->stock ? '1' : '0' }}">
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="width" id="productWidth" value="{{ old('width', $product->width) }}" required 
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative">
                        <select name="has_design" id="hasDesignSelect" class="w-full p-4 rounded-2xl border-none shadow-sm bg-white/80 text-sm outline-none appearance-none cursor-pointer">
                            <option value="no" {{ old('has_design', $product->has_design) ? '' : 'selected' }}>No</option>
                            <option value="si" {{ old('has_design', $product->has_design) ? 'selected' : '' }}>Sí</option>
                        </select>
            ...
            <!-- Sección Diseños -->
            <div id="designVariantsSection" class="{{ old('has_design', $product->has_design) ? '' : 'hidden' }} grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-4 pt-2">
                ...
                <div class="flex flex-col sm:flex-row items-center bg-white/80 rounded-2xl overflow-hidden shadow-sm border border-gray-100 group">
                    ...
                    <span id="mainImageName" class="px-6 py-3 sm:py-0 text-gray-400 text-[11px] italic truncate w-full text-center sm:text-left">Archivo actual preservado</span>
...
document.addEventListener('DOMContentLoaded', function() {
    let state = {
        ...
        selectedCompositions: [
            @foreach($product->compositions as $c)
            { id: "{{ $c->id }}", name: "{{ $c->name }}" },
            @endforeach
        ],
        selectedVariants: [
            @foreach($product->variants->where('category', 'Base') as $v)
            { id: "{{ $v->id }}", name: "{{ $v->name }}", hasOld: {{ $v->pivot->variant_image ? 'true' : 'false' }}, isNewCrop: false },
            @endforeach
        ],
        selectedDesignVariants: [
            @foreach($product->variants->where('category', 'Estampado') as $v)
            { id: "{{ $v->id }}", name: "{{ $v->name }}", hasOld: {{ $v->pivot->variant_image ? 'true' : 'false' }}, isNewCrop: false },
            @endforeach
        ]
    };
    ...
    document.getElementById('confirmCrop').onclick = function() {
        ...
        if (currentTarget === 'main') {
            ...
            document.getElementById('mainImageName').innerText = "Nuevo recorte listo";
        } else {
            ...
            const v = (currentTarget.type === 'base' ? state.selectedVariants : state.selectedDesignVariants).find(x => x.id == currentTarget.id);
            if (v) v.isNewCrop = true;
    ...
    function updateCounts() {
        ...
        let assigned = state.selectedVariants.filter(v => v.hasOld || v.isNewCrop).length;
        if (state.hasDesign === 'si') assigned += state.selectedDesignVariants.filter(v => v.hasOld || v.isNewCrop).length;
    ...
    function renderModal() {
        ...
        const renderRow = (v, type) => {
            ...
            const fileName = v.isNewCrop ? 'Recorte listo' : (v.hasOld ? 'Imagen preservada' : 'Sin archivo');
    ...
    document.getElementById('variantSelect').onchange = (e) => {
        const id = e.target.value; const name = e.target.options[e.target.selectedIndex].text;
        if (id && !state.selectedVariants.find(v => v.id == id)) { state.selectedVariants.push({ id, name, hasOld: false, isNewCrop: false }); createPersistentInput(id, 'base'); renderAllChips(); }
        e.target.value = '';
    };

    document.getElementById('designVariantSelect').onchange = (e) => {
        const id = e.target.value; const name = e.target.options[e.target.selectedIndex].text;
        if (id && !state.selectedDesignVariants.find(dv => dv.id == id)) { state.selectedDesignVariants.push({ id, name, hasOld: false, isNewCrop: false }); createPersistentInput(id, 'design'); renderAllChips(); }
        e.target.value = '';
    };
